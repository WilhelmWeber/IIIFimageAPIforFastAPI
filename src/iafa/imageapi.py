...
class imageAPI:
    ...
    def executor(self, imagePath: str, reqPath: str) -> tuple[bytes, str]:
        paths = self.__reqPathParser(reqPath)
        img = Image.open(imagePath)
        if (not img): raise ValueError('404')

        reg = self.__regionSelector(paths["region"], img.size)
        siz = self.__sizeSelector(reg[3], reg[4], paths["size"])
        rot = self.__angleSelector(paths["rotation"])
        qua = self.__qualitySelector(paths["quality"])
        
        cropped = img.crop((reg[0], reg[1], reg[0]+reg[3], reg[1]+reg[4]))
        sized = cropped.resize(siz)
        if rot["isFlip"] == True:
            sized = sized.transpose(Image.FLIP_LEFT_RIGHT)
        rotated = sized.rotate(rot["angle"])
        if qua:
        ...
        vf = io.BytesIO()
        qualitated.save(vf, format=paths["format"])
        return (vf.getvalue(), f"image/{paths["format"]}")
    
    #return technicalPropaties
    def info(self, imagePath: str, identifier: str) -> dict[str, any]:
        img = Image.open(imagePath)
        metaData = img.size
    ...
    def __regionSelector(self, region: str, metaData: tuple[int, int]) -> tuple[int, int, int, int]:
        ...
        if region == "full":
            ...
            if metaData[0] > metaData[1]:
                ...
                return (margin, 0, metaData[0]-margin, metaData[1])
            else:
                margin: int = (metaData[1] - metaData[2]) /2
                return (0, margin, metaData[0], metaData[1]-margin)
        elif re.match(r'^pct:\d{1,},\d{1,},d{1,},\d{1,}$'):
            xywh = region[4:].split(',')
            x = round(int(xywh[0]) * 0.01 * metaData[0])
            y = round(int(xywh[1]) * 0.01 * metaData[1])
            w = round(int(xywh[2]) * 0.01 * metaData[2])
            h = round(int(xywh[3]) * 0.01 * metaData[3])
            return (x, y, w, h)
        elif re.match(r'^\d{1,},\d{1,},d{1,},\d{1,}$'):
            xywh = region.split(',')
            x = round(int(xywh[0]))
            y = round(int(xywh[1]))
            w = round(int(xywh[2]))
    ...
    def __sizeSelector(self, width: int, height: int, size: str) -> tuple[int, int]:
        if size == "max":
            ...
            if (n > 100):
                raise ValueError('400')
            return (width*0.01*n, height*0.01*n)
        elif re.match(r'^\^pct:\d{1,}$', size):
            n: int = int(size[5:])
            return (width*0.01*n, height*0.01*n)
        #w,h
